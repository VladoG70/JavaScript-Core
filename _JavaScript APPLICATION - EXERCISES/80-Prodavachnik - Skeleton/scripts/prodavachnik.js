function startApp() {
    ...
    function showReadMoreView() {
        $("#readMoreHome").show();
        showView('readMoreHome');
    }
        ...
        function loadAdvSuccess(adverts) {
            ...
            function appendAdvertRow(advert, advTable) {
                ...
                if (advert._acl.creator == sessionStorage['userId']) {
                    let readMoreLink = $('<a href="#">[Read more]</a>').click(function () { readMoreAdvert(advert) });
    ...
    //READ MORE -> DETAILED VIEW Advertisement
    function readMoreAdvert(advert) {
        // $('main > section').hide(); // Hide ALL MENU links
        $("#readMoreHome").empty();
        $.ajax({
            method: "GET",
            url: kinveyBaseUrl + "appdata/" + kinveyAppKey + "/adv/" + advert._id,
            headers: getKinveyUserAuthHeaders(),
            success: loadReadMoreAdvertSuccess,
            error: handleAjaxError
        });

        function loadReadMoreAdvertSuccess(advertByID) {
            let blockAdvert = $('<div>').append(
                $('<label>').text("Title:"),
                $('<h1>').text(advertByID.Title),
                $('<br>'),
                $('<label>').text("Description:"),
                $('<p>').text(advertByID.Description),
                $('<br>'),
                $('<label>').text("Publisher:"),
                $('<p>').text(advertByID.Publisher),
                $('<br>'),
                $('<label>').text("Date:"),
                $('<p>').text(advertByID.Date_of_Publishing),
                $('<br>')
                //$('<label>').text("Views:")
            );

            $('#readMoreHome').append(blockAdvert);
            showView('readMoreHome');
        }
